...
void main() {
  group('Search results sorting', () {
    test('sorts new vines chronologically before original vines with loops', () {
      ...
      expect(sorted[0].id, 'new-recent', reason: 'Most recent new vine should be first');
      expect(sorted[1].id, 'new-older', reason: 'Older new vine should be second');
      expect(sorted[2].id, 'original-zero', reason: 'Zero loops treated as new vine, oldest');
      expect(sorted[3].id, 'original-high', reason: 'Highest loop count should be fourth');
      expect(sorted[4].id, 'original-low', reason: 'Lower loop count should be last');
    });
    ...
    test('sorts vines with equal loops by timestamp', () {
      ...
      expect(sorted[0].id, 'vine1', reason: 'More recent vine should be first when loops equal');
      expect(sorted[1].id, 'vine2', reason: 'Older vine should be second when loops equal');
    });
