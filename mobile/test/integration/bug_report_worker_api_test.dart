...
void main() {
  ...
  group('BugReportService Worker API Integration', () {
    ...
    test('sendBugReport should successfully POST to Worker API', () async {
      ...
      expect(result.reportId, equals(testData.reportId),
          reason: 'Report ID should match');
      expect(result.error, isNull, reason: 'Should not have an error message');

      print('✅ Bug report sent successfully: ${result.reportId}');
      print('   API endpoint: ${BugReportConfig.bugReportApiUrl}');
    }, timeout: const Timeout(Duration(seconds: 30)));

    test('sendBugReport should handle API errors gracefully', () async {
      ...
      print('Result: ${result.success ? "Success" : "Failed: ${result.error}"}');
    }, timeout: const Timeout(Duration(seconds: 30)));

    test('collectDiagnostics and send complete workflow', () async {
