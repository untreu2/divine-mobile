...
void main() {
  ...
  setUp(() async {
    ...
    await Directory(p.join(testDir.path, 'app_support')).create(recursive: true);
  ...
  group('macOS Sandbox Compliance', () {
    test('UploadInitializationHelper uses app support directory, not Documents', () async {
      ...
      expect(hivePath.contains('Documents'), isFalse,
          reason: 'Hive should not use ~/Documents directory on macOS');

      // Verify it uses app_support instead
      expect(hivePath.contains('app_support'), isTrue,
          reason: 'Hive should use ApplicationSupport directory');
    });

    test('Permission errors (EPERM, EACCES) fail immediately without retry', () async {
      ...
      final box = await UploadInitializationHelper.initializeUploadsBox(forceReinit: true);
      expect(box.isOpen, isTrue);
    ...
    test('No file operations attempt to access ~/Documents directory', () async {
      ...
      if (boxPath != null) {
        expect(boxPath.contains('/Documents/'), isFalse,
            reason: 'Box path should not contain /Documents/');
      }
    });

    test('Cold start initialization completes quickly without stalls', () async {
      final startTime = DateTime.now();
    ...
    test('FileSystemException with errno 1 (EPERM) fails without retry', () async {
    ...
    test('FileSystemException with errno 13 (EACCES) fails without retry', () async {
  ...
  group('Regression Guards', () {
    ...
    test('App uses getApplicationSupportDirectory for all persistent storage', () {
