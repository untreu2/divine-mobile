...
void main() {
  ...
  group('SeedMediaPreloadService', () {
    ...
    test('loadSeedMediaIfNeeded skips load when cache already populated', () async {
      ...
      expect(await markerFile.exists(), isTrue, reason: 'Marker file should still exist');
    });

    test('loadSeedMediaIfNeeded copies bundled videos to cache when empty', () async {
      // Setup: Mock manifest.json asset
      final manifestJson = jsonEncode({
        'videos': [
          {
            'eventId': 'test_event_1111111111111111111111111111111111111111111111111111111111111111',
            'filename': 'test_event_1111111111111111111111111111111111111111111111111111111111111111.mp4',
      ...
      final cacheDir = Directory(path.join(tempDir.path, 'openvine_video_cache'));
      final markerFile = File(path.join(cacheDir.path, '.seed_media_loaded'));
      expect(await markerFile.exists(), isTrue, reason: 'Marker file should be created after load');
    });
    ...
    test('loadSeedMediaIfNeeded handles corrupted manifest gracefully', () async {
    ...
    test('loadSeedMediaIfNeeded uses eventId as filename in cache directory', () async {
      // Setup: Mock manifest with specific eventId
      final testEventId = 'unique0000test1111cafe2222beef3333dead4444face5555abcd6666ef0012345678';
      ...
      final testVideoBytes = Uint8List.fromList([0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45]);
      ...
      ServicesBinding.instance.defaultBinaryMessenger.setMockMessageHandler(
        ...
        (ByteData? message) async {
          ...
          if (assetName.contains('manifest.json')) {
            ...
            final bytes = Uint8List.fromList(utf8.encode(manifestJson));
            return ByteData.sublistView(bytes);
          } else if (assetName.contains('unique0000test1111') && assetName.contains('.mp4')) {
      ...
      expect(await videoFile.exists(), isTrue, reason: 'Video file should exist with eventId as filename');

      // Verify file content matches
      final fileBytes = await videoFile.readAsBytes();
      expect(fileBytes, equals(testVideoBytes), reason: 'File content should match asset bytes');
    });
  });
}
