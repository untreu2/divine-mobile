...
void main() {
  ...
  group('BlossomAuthService - GET request auth', () {
    test('createGetAuthHeader returns null when user not authenticated', () async {
      // Arrange
    ...
    test('createGetAuthHeader creates kind 24242 event with correct tags', () async {
      // Arrange
      ...
      when(() => mockEvent.content).thenReturn('Get blob from Blossom server');
    ...
    test('createGetAuthHeader includes server tag when serverUrl provided', () async {
      // Arrange
      when(() => mockAuthService.isAuthenticated).thenReturn(true);
      ...
      expect(tags.any((tag) => tag[0] == 'server' && tag[1] == 'https://blossom.example.com'), isTrue);
    });
    ...
    test('createGetAuthHeader caches tokens to avoid re-signing', () async {
      ...
      // Act
      final result1 = await blossomAuthService.createGetAuthHeader(sha256Hash: 'abc123');
      final result2 = await blossomAuthService.createGetAuthHeader(sha256Hash: 'abc123');
