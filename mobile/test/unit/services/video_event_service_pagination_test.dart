...
void main() {
  ...
  setUp(() {
    ...
    when(mockNostrService.connectedRelayCount).thenReturn(3); // Mock having connected relays
  ...
  group('PaginationState', () {
    ...
    test('should start query by resetting event counter and setting isLoading', () {
      // Arrange
      final paginationState = PaginationState();
      paginationState.eventsReceivedInCurrentQuery = 5; // Simulate previous query
  ...
  group('VideoEventService Pagination', () {
    test('should return early when pagination state has no more content', () async {
    ...
    test('should return early when pagination state is already loading', () async {
      // Arrange
      await videoEventService.subscribeToVideoFeed(
        subscriptionType: SubscriptionType.discovery,
        limit: 10,
  ...
  group('Historical Events Processing', () {
    test('should increment event counter when processing historical events', () async {
      ...
      videoEventService.addVideoEventForTesting(mockEvent1, SubscriptionType.discovery, isHistorical: true);
      videoEventService.addVideoEventForTesting(mockEvent2, SubscriptionType.discovery, isHistorical: true);
      
      // Assert
      expect(discoveryState.eventsReceivedInCurrentQuery, equals(2));
    });

    test('should not increment counter for real-time events during historical query', () async {
      // Arrange
      await videoEventService.subscribeToVideoFeed(
        subscriptionType: SubscriptionType.discovery,
        limit: 10,
      ...
      videoEventService.addVideoEventForTesting(mockEvent, SubscriptionType.discovery, isHistorical: false);
...
Event _createMockNostrEvent(String id, int createdAt) {
  return Event(
    'mock-pubkey-1234567890abcdef1234567890abcdef1234567890abcdef1234567890ab', // 64-char hex pubkey
    32222, // kind
    [
      ['url', 'https://example.com/video.mp4'],
      ['title', 'Mock Video'],
      ['d', 'mock-vine-id'], // Required d tag for kind 32222 events
    ], // tags
