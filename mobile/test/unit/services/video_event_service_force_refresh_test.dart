  ...
  group('VideoEventService Force Refresh', () {
    ...
    setUp(() {
      ...
      when(() => mockNostrService.subscribeToEvents(
        ...
        Future.microtask(() {
          final onEose = invocation.namedArguments[const Symbol('onEose')] as void Function()?;
          onEose?.call();
        });
    ...
    test('force refresh should preserve existing videos, not clear them', () async {
      ...
      when(() => event1.id).thenReturn('video1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
      when(() => event1.kind).thenReturn(34236);
      when(() => event1.pubkey).thenReturn('author1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
      when(() => event1.content).thenReturn('First video');
      when(() => event1.createdAt).thenReturn(DateTime.now().millisecondsSinceEpoch ~/ 1000);
      when(() => event1.tags).thenReturn([
        ['url', 'https://example.com/video1.mp4'],
        ['m', 'video/mp4'],
      ]);
      when(() => event1.sig).thenReturn('sig1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnooooppppqqqqrrrrssssttttuuuuvvvvwwwwxxxxyyyyzzzz00001111222233334444');

      final event2 = MockEvent();
      when(() => event2.id).thenReturn('video2_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
      when(() => event2.kind).thenReturn(34236);
      when(() => event2.pubkey).thenReturn('author2_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
      when(() => event2.content).thenReturn('Second video');
      when(() => event2.createdAt).thenReturn(DateTime.now().millisecondsSinceEpoch ~/ 1000);
      when(() => event2.tags).thenReturn([
        ['url', 'https://example.com/video2.mp4'],
        ['m', 'video/mp4'],
      ]);
      when(() => event2.sig).thenReturn('sig2_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnooooppppqqqqrrrrssssttttuuuuvvvvwwwwxxxxyyyyzzzz00001111222233334444');
      ...
      final videosAfterFirst = videoEventService.getVideos(SubscriptionType.popularNow);
      expect(videosAfterFirst.length, 1);
      expect(videosAfterFirst[0].id, 'video1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
      ...
      final videosAfterRefresh = videoEventService.getVideos(SubscriptionType.popularNow);
      ...
      expect(videosAfterRefresh.length, 2,
        reason: 'Force refresh should preserve existing videos and add new ones');

      // Verify both videos are present
      final videoIds = videosAfterRefresh.map((v) => v.id).toList();
      expect(videoIds, contains('video1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp'),
        reason: 'Original video should still be present after force refresh');
      expect(videoIds, contains('video2_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp'),
    ...
    test('force refresh should not duplicate existing videos', () async {
      ...
      expect(videos[0].id, 'video1_aaaabbbbccccddddeeeeffffgggghhhhiiiijjjjkkkkllllmmmmnnnnoooopppp');
    });
  });
}
